...
class Queue {
  private:
    unsigned long _front, _back, _count;
    T *_data;
    unsigned long _maxitems;
  public:
    Queue(unsigned long maxitems = 256) { 
      _front = 0;
      _back = 0;
      _count = 0;
      _maxitems = maxitems;
...
template<class T>
unsigned long Queue<T>::count() 
{
  return _count;
}

template<class T>
unsigned long Queue<T>::front() 
{
  return _front;
}

template<class T>
unsigned long Queue<T>::back() 
{
  return _back;
}
